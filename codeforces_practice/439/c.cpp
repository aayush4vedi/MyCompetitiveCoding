...
// }
// lli nCr(int n,int r){
//   return (((fact(n))%MODULO)/(((fact(r)%MODULO)))*(fact(n-r)%MODULO));
// }
// int main(){
//   DRIII(a,b,c);
//   int ab = min(a,b);int bc = min(c,b);int ac = min(a,c);
//   int ans1,ans2,ans3,ans;
//   REP(i,ab){
//     ans1 += (nCr(a,i)%MODULO)*(nCr(b,i)%MODULO)*(fact(i)%MODULO);
//   }
//   REP(i,bc){
//     ans2 += (nCr(b,i)%MODULO)*(nCr(c,i)%MODULO)*(fact(i)%MODULO);
//   }
//   REP(i,ac){
//     ans1 += (nCr(a,i)%MODULO)*(nCr(c,i)%MODULO)*(fact(i)%MODULO);
//   }
//   ans = ans1+ans2+ans3;
//   cout<<ans%MODULO;
//   NL;
