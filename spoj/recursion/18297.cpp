...
int dpS(int i,int j){
  ...
  if(dp2[i][j] != -1)return dp2[i][j];
    ...
    ans=max((a[i]+max((dpS(i+1,j-1)),(dpS(i+2,j)))),(a[j]+max((dpS(i+1,j-1)),(dpS(i,j-2)))));
  }
  return ans;
}
int dpD(int i, int j){
  ...
  if(dp1[i][j] != -1)return dp1[i][j];
    ...
    ans = max((a[i]+min(dpD(i+2,j),dpD(i+1,j-1))),(a[j]+min(dpD(i+1,j-1),dpD(i,j-2))));
  }
  return ans;
}
