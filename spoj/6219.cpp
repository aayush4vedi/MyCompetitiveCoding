...
const int MAX = 1e9+1;
/*
int edist(string a, string b, int n, int m){
  if(n==0 && m==0)return 0;
  if(n==0)return m;
  if(m==0)return n;
  if(a[n-1]==b[m-1])return(a,b,n-1,m-1);
  return (1 + min(edist(a,b,n-1,m-1),min(edist(a,b,n,m-1),edist(a,b,n-1,m))));
}
*/
int edist(string a, string b, int la, int lb){
  int dp[la+1][lb+1];
  REP(i,0,la+1){
    REP(j,0,lb+1){
      if(i==0 && j==0)dp[i][j]=0;
