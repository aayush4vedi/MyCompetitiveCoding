...
bool isNotSafe(int board[][10001],int n,int x,int y){
  REP(i,0,n){
  ...
  REP(i,0,n){
    REP(j,0,n){
      if(j-i==x-y)return true;
      else if(i+j==y+x)return true;
    }
  }
  return false;
}
bool nQueen(int b[][10001],int n){
  if(n==0)return true;
  REP(i,0,n){
    REP(j,0,n){
      while(isNotSafe(board,n,i,j)){
        if(j<n-1)j++;
        else if(j==n-1)i++;
        //cout<<"here\n";
      }
...
int main(){
 ...
 if(nQueen(board,n)==true){cout<<"YES\n";}
 else cout<<"NO\n";
 REP(i,0,n){
   REP(j,0,n){
     cout<<board[i][i]<<" ";
